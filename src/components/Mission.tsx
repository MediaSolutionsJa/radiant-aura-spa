    ...
    <section className="section-dark py-16 md:py-24">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2 className="font-lavish text-3xl md:text-4xl text-spa-dark-foreground mb-8">
          {t('missionTitle').split(' ')[0]} <span className="text-luxury-gold">{t('missionTitle').split(' ').slice(1).join(' ')}</span>
        </h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-6 mt-12">
          {['values.luxury', 'values.comfort', 'values.confidence', 'values.wellness', 'values.glow'].map((key, index) => (
            <div key={index} className="text-center">
              <div className="w-16 h-16 bg-gradient-luxury rounded-full flex items-center justify-center mx-auto mb-3 shadow-luxury">
                <span className="text-luxury-gold-foreground font-semibold text-sm">{t(key).charAt(0)}</span>
              </div>
              <p className="text-spa-dark-foreground/80 font-medium text-sm">{t(key)}</p>
            </div>
          ))}
        </div>
      </div>
